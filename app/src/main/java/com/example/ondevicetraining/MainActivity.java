        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    // olaf_reddit();
                    // mobileNet(1, new int[]{224, 224, 3});
                    mobileNetV2(1, new int[]{224, 224, 3});
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
...
//            net.output(features);
    ...
    // A sequence of 1 or more identical (modulo stride) bottleneck layers, repeated n times.
    private void bottleneckConv(ComputationGraphConfiguration.GraphBuilder graph,
                                int[] args, String id, String input) {
        // Bottleneck residual block transforming from input_c to c channels, with stride s, and expansion factor t.
        int t = args[0], c = args[1], n = args[2], s = args[3], input_c = args[4];
        String next_input, temp_id;
        // the first bottleneck
        graph.addLayer(id, new Convolution2D.Builder().convolutionMode(ConvolutionMode.Same).kernelSize(1, 1).stride(1, 1).nOut(t * input_c).build(), input)
                .addLayer(id + "BN", new BatchNormalization.Builder().build(), id)
                .addLayer(id + "Relu6", new ActivationLayer.Builder().activation(Activation.RELU6).build(), id + "BN")

                .addLayer(id + "dw", new SeparableConvolution2D.Builder().convolutionMode(ConvolutionMode.Same).kernelSize(3, 3).stride(s, s).nOut(t * input_c).depthMultiplier(1).build(), id + "Relu6")
                .addLayer(id + "dwBN", new BatchNormalization.Builder().build(), id + "dw")
                .addLayer(id + "dwRelu6", new ActivationLayer.Builder().activation(Activation.RELU6).build(), id + "dwBN")

                .addLayer(id + "pw", new Convolution2D.Builder().convolutionMode(ConvolutionMode.Same).kernelSize(1, 1).stride(1, 1).nOut(c).build(), id + "dwRelu6")
                .addLayer(id + "pwBN", new BatchNormalization.Builder().build(), id + "pw");
        next_input = id + "pwBN";
        // other bottlenecks
        for (int i = 1; i < n; ++i) {
            temp_id = "_" + i;
            graph.addLayer(id + temp_id, new Convolution2D.Builder().convolutionMode(ConvolutionMode.Same).kernelSize(1, 1).stride(1, 1).nOut(t * c).build(), next_input)
                    .addLayer(id + temp_id + "BN", new BatchNormalization.Builder().build(), id + temp_id)
                    .addLayer(id + temp_id + "Relu6", new ActivationLayer.Builder().activation(Activation.RELU6).build(), id + temp_id + "BN")

                    .addLayer(id + temp_id + "dw", new SeparableConvolution2D.Builder().convolutionMode(ConvolutionMode.Same).kernelSize(3, 3).stride(1, 1).nOut(t * c).depthMultiplier(1).build(), id + temp_id + "Relu6")
                    .addLayer(id + temp_id + "dwBN", new BatchNormalization.Builder().build(), id + temp_id + "dw")
                    .addLayer(id + temp_id + "dwRelu6", new ActivationLayer.Builder().activation(Activation.RELU6).build(), id + temp_id + "dwBN")

                    .addLayer(id + temp_id + "pw", new Convolution2D.Builder().convolutionMode(ConvolutionMode.Same).kernelSize(1, 1).stride(1, 1).nOut(c).build(), id + temp_id + "dwRelu6")
                    .addLayer(id + temp_id + "pwBN", new BatchNormalization.Builder().build(), id + temp_id + "pw")

                    // the residual structure
                    .addVertex(id + temp_id + "res", new ElementWiseVertex(ElementWiseVertex.Op.Add), next_input, id + temp_id + "pwBN");
            next_input = id + temp_id + "res";
        }
    }

    private void mobileNetV2(int batch_size, int[] input_shape) {
        // input_shape = {h, w, c}
        ComputationGraphConfiguration.GraphBuilder graphBuilder = new NeuralNetConfiguration.Builder()
                ...
                .updater(new Nesterovs(0.1, 0.9))
                .graphBuilder();
        graphBuilder.addInputs("input").setInputTypes(InputType.convolutionalFlat(input_shape[0], input_shape[1], input_shape[2]))
                // conv2d
                .addLayer("0", new Convolution2D.Builder().convolutionMode(ConvolutionMode.Same).kernelSize(3, 3).stride(2, 2).nOut(32).build(), "input")
                .addLayer("0BN", new BatchNormalization.Builder().build(), "0")
                .addLayer("0Relu6", new ActivationLayer.Builder().activation(Activation.RELU6).build(), "0BN");
        int[][] argsMatrix = new int[][] {
                {1, 16, 1, 1, 32},
                {6, 24, 2, 2, 16},
                {6, 32, 3, 2, 24},
                {6, 64, 4, 2, 32},
        ...
        // bottlenecks
        // 1
        bottleneckConv(graphBuilder, argsMatrix[0], "1", "0Relu6");
        // 2
        bottleneckConv(graphBuilder, argsMatrix[1], "2", "1pwBN");
        // 3
        bottleneckConv(graphBuilder, argsMatrix[2], "3", "2_1res");
        // 4
        bottleneckConv(graphBuilder, argsMatrix[3], "4", "3_2res");
        // 5
        bottleneckConv(graphBuilder, argsMatrix[4], "5", "4_3res");
        // 6
        bottleneckConv(graphBuilder, argsMatrix[5], "6", "5_2res");
        // 7
        bottleneckConv(graphBuilder, argsMatrix[6], "7", "6_2res");
        // conv2d 1*1
        graphBuilder.addLayer("8", new Convolution2D.Builder().convolutionMode(ConvolutionMode.Same).kernelSize(1, 1).stride(1, 1).nOut(1280).build(), "7pwBN")
                .addLayer("8BN", new BatchNormalization.Builder().build(), "8")
                .addLayer("8Relu6", new ActivationLayer.Builder().activation(Activation.RELU6).build(), "8BN")
                // last layers
                .addLayer("avg_pool", new GlobalPoolingLayer.Builder().poolingType(PoolingType.AVG).build(), "8Relu6")
                .addLayer("fc", new DenseLayer.Builder().activation(Activation.SOFTMAX).nOut(100).build(), "avg_pool")
                .addLayer("output", new OutputLayer.Builder().activation(Activation.SOFTMAX).lossFunction(LossFunctions.LossFunction.SPARSE_MCXENT).nOut(10).build(), "fc")
                .setOutputs("output");

        ComputationGraphConfiguration graphConf = graphBuilder
                .backpropType(BackpropType.Standard)
                .build();
        ComputationGraph graph = new ComputationGraph(graphConf);
        graph.init();

        INDArray features = Nd4j.create(batch_size, input_shape[2], input_shape[1], input_shape[0]);
        INDArray labels = Nd4j.create(batch_size, 1);
        ...
        String filename = "MLN_mobileNetV2.txt";
        File iris_dir = new File(Environment.getExternalStorageDirectory(), "iris_classifier");
        if (!iris_dir.exists()) {
            iris_dir.mkdirs();
        }
